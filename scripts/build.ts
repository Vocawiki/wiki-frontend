...
async function buildEntity({ type, name, path }: { type: 'widget' | 'gadget'; name: string; path: string }) {
  ...
  const widgetCode = getWidgetCode({ name, description, srcPath: path, script: sourceCode })
...
await Promise.all(
  widgetEntries.map((entity) => {
    buildEntity({ type: 'widget', ...entity })
  }),
)
