    ...
    class AIVCWindow extends OO.ui.ProcessDialog {
                ...
                this.confirmPanel.$element.append(new OO.ui.FieldLayout(new OO.ui.Widget({
                    content: [
                        new OO.ui.HorizontalLayout({
                            items: this.config.main.map((v) => new OO.ui.ButtonWidget({
        ...
        async getVariants(original) {
            ...
            for (const variant of this.config.main) {
                ...
                results[variant] = restored;
            }

            // 1. zh-hans ← zh-cn
            this.addVariant("zh-hans", results["zh-cn"]);

            // 2. zh-hant ← zh-tw
            this.addVariant("zh-hant", results["zh-tw"]);

            // 3. 添加 zh-hk，提交时再行判断
            this.addVariant("zh-hk", results["zh-hk"]);
        }
        async saveChanges() {
            // Occasionally edits are not saved - keep this log until the bug is identified
            console.log("[VariantConverter] Saved changes", await Promise.allSettled(Object.keys(this.textInputs).map((variant) => {
                ...
                return api.postWithToken("csrf", {
                    ...
                    nocreate: variant === "zh-hk" && text === this.textInputs["zh-hant"].getValue() ? "1" : undefined,
                });
            })));
        }
    }
