...
$(async () => {
	...
	const restoreWikitext = (original, mappings) => {
		...
		replaced = replaced.replace(
			REGEXP.extLink,
			(_, before, target, text, after) => `${before}[${mappings[target]}${text || ''}]${after}`,
		)
			...
			for (const variant of this.config.main) {
				...
				if (this.config.useOpenCC) {
					...
					converted = converted.replace(
						REGEXP.noOCC,
						(_, content) => `<!--noOCC-->${occMappings.push(content) - 1}<!--/noOCC-->`,
					)
		...
		async saveChanges() {
			...
			console.log(
				...
				await Promise.allSettled(
					Object.keys(this.textInputs).map((variant) => {
						...
						return api.postWithToken('csrf', {
							...
							nocreate:
								variant === 'zh-hk' && text === this.textInputs['zh-hant'].getValue()
									? '1'
									: undefined,
						})
					}),
